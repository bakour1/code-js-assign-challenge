...
fetch("https://www.breakingbadapi.com/api/") //جلب
    .then(function(respone){            //من ثم انشاء فانكسن
        console.log(respone)            //print the func in consol
        return respone.json();          // print in json
    })
    
    .then(function(data){               // then  fetch all "response" to data and print it
        console.log(data)
    })

//  the new
async function get(){ //async => synce 
    ...
    data.map(function(actors){ // aftar i was collected all informtion in variable  "data" i progrming map for work with all masfofa for gave me all name
        console.log(actors.name)
    })

    document.querySelector("#content h1").innerHTML = "<h1>"+ data[0].name +"</h1>" //select the id for div and chang ""all"" content div by .innerHtml to consol.log "test"
    ...
    `<select>
    ${
        data.map(actors => `<option>${actors.name}</option>`) // now i creat one select with map data option
    }
    </select>`
}
get() //call the func
